...
describe('BFSPlugin', () => {
  ...
  it('should visit all nodes in the triangle graph', () => {
    ...
    const nodeHighlights = trace.events.filter(e => e.type === 'GRAPH_NODE_HIGHLIGHT');
    const visitedNodes = new Set(nodeHighlights.map(e => e.type === 'GRAPH_NODE_HIGHLIGHT' ? e.nodeId : ''));
  ...
  it('should emit GRAPH_EDGE_HIGHLIGHT events', () => {
    const trace = plugin.execute({ ...triangleGraph, startNodeId: 'A' });
    const edgeEvents = trace.events.filter(e => e.type === 'GRAPH_EDGE_HIGHLIGHT');
  ...
  it('should start from the specified startNodeId', () => {
    ...
    expect(nodeHighlights[0].type === 'GRAPH_NODE_HIGHLIGHT' && nodeHighlights[0].nodeId).toBe('C');
  });

  it('should handle empty graph gracefully', () => {
    const trace = plugin.execute({ nodes: [], edges: [] });
    const errorLogs = trace.events.filter(e => e.type === 'SYSTEM_LOG' && e.level === 'ERROR');
...
describe('PrimPlugin', () => {
  ...
  it('should emit a completion SYSTEM_LOG', () => {
    const trace = plugin.execute(weightedGraph);
    const logs = trace.events.filter(e => e.type === 'SYSTEM_LOG' && e.level === 'INFO');
...
describe('TopoSortPlugin', () => {
  ...
  it('should detect a cycle in a cyclic graph', () => {
    ...
    const warnLogs = trace.events.filter(e => e.type === 'SYSTEM_LOG' && e.level === 'WARN');
