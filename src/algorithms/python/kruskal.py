...
def kruskal(edges, n):
    ...
    if edges_accepted < n - 1:
        emit(
            "SYSTEM_LOG",
            message="MST cannot be formed: the graph is disconnected.",
            level="WARN",
        )
    else:
        emit(
            "SYSTEM_LOG",
            message=f"Minimum Spanning Tree found with total weight: {mst_weight}",
            level="INFO",
        )
    return mst_weight
...
def generate_graph_payload(nodes, edges):
    ...
    payload_edges = [
        {"id": e[2], "from": f"n{e[0]}", "to": f"n{e[1]}", "weight": e[3]}
        for e in edges
    ]
