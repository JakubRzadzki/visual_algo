...
def dijkstra(src, graph):
    ...
    if unreachable > 0:
        emit("SYSTEM_LOG", message=f"Warning: {unreachable} node(s) are unreachable from the source n{src}.", level="WARN")
        
    emit("SYSTEM_LOG", message=f"Final shortest paths from n{src}: " + ", ".join(results), level="INFO")
    return dist

def generate_graph_payload(graph_adj):
    nodes = [{"id": f"n{i}", "label": str(i), "x": (i%2)*150 - 75, "y": (i//2)*150 - 75} for i in range(len(graph_adj))]
    edges = []
