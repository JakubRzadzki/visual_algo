...
def a_star(width, height, start, target, walls):
    push_event(
        "SYSTEM_LOG",
        {"message": f"Starting A* Search from {start} to {target}", "level": "INFO"},
    )
    ...
    while open_set:
        ...
        if current != start and current != target:
            push_event(
                ...
                {"row": current[1], "col": current[0], "color": "#06b6d4"},
            )
            push_event(
                "MATRIX_CELL_UPDATE", {"row": current[1], "col": current[0], "value": f}
            )
        ...
        if current == target:
            ...
            while curr in came_from:
                ...
                if curr != start and curr != target:
                    push_event(
                        "MATRIX_CELL_HIGHLIGHT",
                        {"row": curr[1], "col": curr[0], "color": "#eab308"},
                    )
            return path
