...
type CellState = 'empty' | 'wall' | 'start' | 'end' | 'visited' | 'path' | 'active';
...
export default function GridStage() {
      ...
      switch (event.type) {
        case 'TRACE_LOADED':
          setGrid(prev => prev.map(row => row.map(cell => {
            if (cell.type === 'visited' || cell.type === 'path' || cell.type === 'active') {
              return { ...cell, type: 'empty', fScore: undefined, gScore: undefined, hScore: undefined };
            }
            return cell;
          })));
          setStats({ visitedCount: 0, pathLength: 0, currentF: 0, currentG: 0, currentH: 0, activeCoord: null });
          break;
          ...
          setGrid(prev => {
            ...
            if (cell.type !== 'start' && cell.type !== 'end' && cell.type !== 'wall') {
              next[row][col] = { 
                ...
                parent: dependencies?.[0] ? { x: dependencies[0][1], y: dependencies[0][0] } : undefined
              };
            }
            return next;
          });
  ...
  const handleMouseEnter = (x: number, y: number) => {
    ...
    if (draggedNodeType === 'start') {
      ...
      if (grid[y][x].type !== 'wall' && (x !== startPos.x || y !== startPos.y)) {
        setEndPos({ x, y });
      }
    } else {
    ...
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="glass-panel p-4 rounded-xl border border-cyan-500/10 flex items-center gap-4">
          ...
          <div>
            <div className="text-[10px] text-slate-500 uppercase font-bold tracking-wider">Visited Cells</div>
            <div className="text-xl font-mono text-cyan-500">{stats.visitedCount}</div>
          </div>
        </div>
        ...
        <div className="glass-panel p-4 rounded-xl border border-emerald-500/10 flex items-center gap-4">
          ...
          <div>
            <div className="text-[10px] text-slate-500 uppercase font-bold tracking-wider">Path Length</div>
            <div className="text-xl font-mono text-emerald-500">{stats.pathLength}</div>
          </div>
        </div>
        ...
        <div className="glass-panel p-4 rounded-xl border border-amber-500/10 col-span-1 md:col-span-2 flex items-center justify-between px-6">
          <div className="flex items-center gap-6">
            <div className="flex flex-col">
              <span className="text-[10px] text-slate-500 uppercase font-bold tracking-wider">Active Cell</span>
              <span className="text-lg font-mono text-amber-400">
                {stats.activeCoord ? `(${stats.activeCoord.x}, ${stats.activeCoord.y})` : 'N/A'}
              </span>
            </div>
            {activeAlgorithmId === 'a-star' && (
              <div className="flex gap-4 border-l border-white/5 pl-6">
                <div className="flex flex-col">
                  <span className="text-[10px] text-slate-500 uppercase font-bold tracking-wider">f-score</span>
                  <span className="text-sm font-mono text-white">{stats.currentF || '—'}</span>
                </div>
                <div className="flex flex-col">
                  <span className="text-[10px] text-slate-500 uppercase font-bold tracking-wider">g-score</span>
                  <span className="text-sm font-mono text-slate-400">{stats.currentG || '—'}</span>
                </div>
              </div>
            )}
          </div>
...
function GridCellComponent({ 
  x, y, cell, onMouseDown, onMouseEnter 
}: { 
  x: number, y: number, cell: GridCell, 
  ...
  const getVariants = () => {
    switch (cell.type) {
      case 'visited': return {
        ...
        animate: { scale: 1, borderRadius: '4px', opacity: 1, backgroundColor: 'rgba(6, 182, 212, 0.3)' }
      };
      case 'path': return {
  ...
  return (
    ...
    >
      <AnimatePresence mode="wait">
        {(cell.type === 'visited' || cell.type === 'path' || cell.type === 'active') && (
          <motion.div 
            key={cell.type}
