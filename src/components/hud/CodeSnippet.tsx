  ...
  while (remaining.length > 0) {
    ...
    if (!matched) {
      const keywordMatch = remaining.match(
        new RegExp(`^\\b(${colors.keywords.join("|")})\\b`),
      );
      if (keywordMatch) {
        result.push(
          <span
            key={`keyword-${tokenId++}`}
            style={{ color: COLORS.keyword, fontWeight: "500" }}
          >
            {keywordMatch[1]}
          </span>,
        );
    ...
    if (!matched) {
      const functionMatch = remaining.match(
        new RegExp(`^\\b(${colors.functions.join("|")})\\b`),
      );
      if (functionMatch) {
        result.push(
          <span
            key={`function-${tokenId++}`}
            style={{ color: COLORS.function, fontWeight: "500" }}
          >
            {functionMatch[1]}
          </span>,
        );
...
export default function CodeSnippet() {
  ...
  const code =
    ALGORITHMS[algoName]?.[langKey] || ALGORITHMS["Merge Sort"][langKey];
