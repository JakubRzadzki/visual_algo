...
export default function Sidebar(): React.ReactElement {
  ...
  const sortingAlgos =
    sortingCategory?.algorithms.filter((a) => a.available) || [];
  const searchingAlgos =
    searchingCategory?.algorithms.filter((a) => a.available) || [];
  ...
  const getTreeIcon = (type: TreeType) => {
    switch (type) {
      case "binary":
        return (
          <GitMerge size={18} strokeWidth={2} className="text-amber-400" />
        );
      case "bst":
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-wrap rounded-xl overflow-hidden border border-glacier-border-bright bg-glacier-surface p-1 gap-0.5 justify-center">
        {(["sorting", "searching", "graph", "tree", "dp", "grid"] as const).map(
          (mode) => (
            <button
              key={mode}
              onClick={() => {
                ...
                if (mode === "searching")
                  navigate("/algo/searching/binary-search");
            ...
            {[
              ...
              >
                ...
                <span className="truncate">
                  {language === "en" ? tree.name : tree.name_pl}
                </span>
              </motion.li>
            ))}
          </ul>
