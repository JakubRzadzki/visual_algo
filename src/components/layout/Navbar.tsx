      ...
      <div className="flex items-center gap-4">
        ...
        <Link to="/" className="font-bold text-lg luminous-text flex items-center gap-2 hover:opacity-80 transition-opacity">
          ...
          <span className="bg-gradient-to-r from-white via-slate-100 to-cyan-400 bg-clip-text text-transparent">
            {useUIStore.getState().language === 'en' ? 'Algorithm Visualizer EDVR' : 'Wizualizator Algorytmów EDVR'}
          </span>
        </Link>
      </div>
        ...
        <div className="flex items-center gap-1 bg-white/5 p-1 rounded-lg border border-white/5 ml-2">
          <button
            ...
            className={`px-2 py-1 rounded text-[10px] font-bold transition-all ${
              useUIStore(state => state.language) === 'en'
          ...
          <button
            ...
            className={`px-2 py-1 rounded text-[10px] font-bold transition-all ${
              useUIStore(state => state.language) === 'pl'
                ? 'bg-cyan-500 text-white shadow-[0_0_10px_rgba(6,182,212,0.4)]'
                : 'text-slate-500 hover:text-slate-300'
            }`}
          >
