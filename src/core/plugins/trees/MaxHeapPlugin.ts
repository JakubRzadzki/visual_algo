...
export class MaxHeapPlugin implements AlgorithmPlugin<ArrayInput> {
  ...
  execute(data: ArrayInput): ExecutionTrace {
    const values = data.values && data.values.length > 0 ? data.values : [15, 30, 20, 45, 10, 50];
    ...
    for (let i = 0; i < finalHeap.length; i++) {
      ...
      if (i > 0) {
        ...
        edges.push({ id: `e${parent}-${i}`, from: `n${parent}`, to: `n${i}`, hidden: true });
      }
    }

    // 2. Animate the insertion
    push({ type: 'SYSTEM_LOG', level: 'INFO', message: 'Starting Max Heap construction (insertions).' });

    const simHeap: { id: string; val: number }[] = [];

    values.forEach((val, index) => {
      push({ type: 'SYSTEM_LOG', level: 'INFO', message: `Inserting ${val} at the end.` });
      ...
      if (index > 0) {
        ...
        push({ type: 'GRAPH_EDGE_ADD', edgeId: `e${parent}-${index}`, from: `n${parent}`, to: `n${index}` });
      }
      ...
      while (curr > 0) {
        const parent = Math.floor((curr - 1) / 2);
        push({ type: 'GRAPH_NODE_HIGHLIGHT', nodeId: simHeap[parent].id, status: 'queued' });
        push({ type: 'SYSTEM_LOG', level: 'INFO', message: `Comparing ${simHeap[curr].val} with parent ${simHeap[parent].val}` });
        
        if (simHeap[curr].val > simHeap[parent].val) {
          push({ type: 'SYSTEM_LOG', level: 'WARN', message: `Swapping ${simHeap[curr].val} and ${simHeap[parent].val}` });
          ...
          curr = parent;
          push({ type: 'GRAPH_NODE_HIGHLIGHT', nodeId: simHeap[curr].id, status: 'current' });
        } else {
          break;
        }
      }
      
      push({ type: 'GRAPH_NODE_HIGHLIGHT', nodeId: simHeap[curr].id, status: 'visited' });
    });
