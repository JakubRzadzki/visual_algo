...
export class UnionFindPlugin implements AlgorithmPlugin<ArrayInput> {
  ...
  execute(data: ArrayInput): ExecutionTrace {
    ...
    const values = data.values && data.values.length > 0 && data.values.length % 2 === 0 
      ? data.values 
      : [0,1, 1,2, 3,4, 4,5, 2,5];
    ...
    visualEdges.forEach(e => {
      edges.push({ id: `e${e.from}-${e.to}`, from: `n${e.from}`, to: `n${e.to}`, hidden: true });
    });

    const initialGraph: GraphInput = {
      nodes: nodes.map(nd => ({ id: nd.id, label: nd.label, x: 0, y: 0, vx: 0, vy: 0, hidden: true })),
    ...
    push({ type: 'SYSTEM_LOG', level: 'INFO', message: 'Initializing disjoint sets.' });
    ...
    const simFind = (i: number): number => {
      push({ type: 'GRAPH_NODE_HIGHLIGHT', nodeId: `n${i}`, status: 'current' });
      if (simParent[i] !== i) {
        push({ type: 'SYSTEM_LOG', level: 'INFO', message: `Path compression for ${i}` });
    ...
    const simUnion = (i: number, j: number) => {
      ...
      if (rootI !== rootJ) {
        if (simRank[rootI] < simRank[rootJ]) {
          ...
          push({ type: 'GRAPH_EDGE_ADD', edgeId: `e${rootI}-${rootJ}`, from: `n${rootI}`, to: `n${rootJ}` });
          push({ type: 'SYSTEM_LOG', level: 'INFO', message: `${rootI} becomes child of ${rootJ}` });
        } else if (simRank[rootI] > simRank[rootJ]) {
          simParent[rootJ] = rootI;
          push({ type: 'GRAPH_EDGE_ADD', edgeId: `e${rootJ}-${rootI}`, from: `n${rootJ}`, to: `n${rootI}` });
          push({ type: 'SYSTEM_LOG', level: 'INFO', message: `${rootJ} becomes child of ${rootI}` });
        } else {
          simParent[rootJ] = rootI;
          simRank[rootI]++;
          push({ type: 'GRAPH_EDGE_ADD', edgeId: `e${rootJ}-${rootI}`, from: `n${rootJ}`, to: `n${rootI}` });
          push({ type: 'SYSTEM_LOG', level: 'INFO', message: `${rootJ} becomes child of ${rootI}, rank of ${rootI} increases` });
        }
      } else {
        push({ type: 'SYSTEM_LOG', level: 'WARN', message: `${i} and ${j} are already in the same set.` });
      }
    };
